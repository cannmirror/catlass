...
class TestMLAPrepross(op_test.OpTest):
    ...
    def golden_compare(self, out_tensors, golden_out_tensors):
        if self.op_desc["specificParam"]["cacheMode"] == 0:
            ...
            max_diff = torch.max(torch.abs(out_tensors[0].flatten() - self.qOutNopeQuant.flatten())) #outtensor npu输出 qOutNopeQuant cpu？小算子？
            result_double1 = max_diff <= 1

            max_diff = torch.max(torch.abs(self.keyCache1_out.flatten() - out_tensors[1].flatten())) #keyCache1_out cpu+npu  out_tensors npu
            result_double2 = max_diff <= 1

            result_double3 = compare_cv(self.qOut_npu[..., 512:576], self.qOut[..., 512:576], out_tensors[2]) or\
                             self.compare_data(self.qOut_npu[..., 512:576].flatten(), out_tensors[2].flatten()) #qrope qout npu小算子拼接输出 qout cpu
            
            result_double4 = self.compare_data(self.keyCache2_out.flatten(), out_tensors[3].flatten()) #keyCache2_out
            return result_double1 and result_double2 and result_double3 and result_double4
