...
public:
    ...
    ~BlockMmad()
    {
        for (uint32_t i = 0; i < L1_STAGES; ++i) {
            ...
            AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(scaleEventList[i]);
        }
    ...
    )
    {
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; ++kLoopIdx) {
            ...
            AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(scaleEventList[l1ListId]);
            copyGmToL1Scale(l1ScaleBufList[l1ListId], gmKTileScale, layoutL1KTileScale, layoutKTileScale);
            AscendC::SetFlag<AscendC::HardEvent::MTE2_FIX>(scaleEventList[l1ListId]);

            // Copy scale from L1 to FixpipeBuffer
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_FIX>(scaleEventList[l1ListId]);
            LayoutFpBuf layoutFpBuf{L1TileShape::N};
            copyL1ToFP(fixpipeBufList[l1ListId], l1ScaleBufList[l1ListId], layoutFpBuf, layoutL1KTileScale);
...
private:
    ...
    void InitL1(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart)
    {
        ...
        for (uint32_t i = 0; i < L1_STAGES; ++i) {
            ...
            l1ScaleBufList[i] = resource.l1Buf.template GetBufferByByte<uint64_t>(l1ScaleOffset +
                SCALE_KTILE_SIZE * i);
            l1AEventList[i] = i;
            ...
            AscendC::SetFlag<AscendC::HardEvent::FIX_MTE2>(scaleEventList[i]);
        }
    }
    ...
    void InitFpBuf(Arch::Resource<ArchTag> &resource, uint32_t FpAddrStart)
    {
        ...
        for (uint32_t i = 0; i < L1_STAGES; ++i) {
            fixpipeBufList[i] = resource.fpBuf.template GetBufferByByte<uint64_t>(FpOffset + SCALE_KTILE_SIZE * i);
        }
    }
    ...
    void L1TileMmad(L1TileMmadParams const &params)
    {
        ...
        auto &fixpipeBuf = fixpipeBufList[params.l1ListId];
        ...
        if (params.isKLoopFirst) {
            if constexpr (!ENABLE_UNIT_FLAG) {
                ...
                AscendC::SetFlag<AscendC::HardEvent::FIX_MTE2>(scaleEventList[l1ListId]);
            } else {
    ...
    AscendC::LocalTensor<uint64_t> fixpipeBufList[L1_STAGES];
    int32_t scaleEventList[L1_STAGES];
