...
/// Block swizzleing function for hybrid splitk matmul
template <uint32_t SwizzleOffset = 1, uint32_t SwizzleDirection = 0>
struct HybridSplitkGemmIdentityBlockSwizzle {
    /// Data members
    ...
    /// Methods
    CATLASS_DEVICE
    HybridSplitkGemmIdentityBlockSwizzle() {}

    /// Methods
    CATLASS_DEVICE
    HybridSplitkGemmIdentityBlockSwizzle(GemmCoord const &problemShape_, GemmCoord const &l1TileShape, uint32_t splitkFactor_)
        : problemShape(problemShape_), tileMNK(l1TileShape), splitkFactor(splitkFactor_)
    {
        ...
        if (splitkSliceIdx < loopsMNK.k() % splitkFactor) {
            ...
            kIdxSliceLen = loopsMNK.k() / splitkFactor + 1;
        } else {
            kIdxSliceStart = splitkSliceIdx * (loopsMNK.k() / splitkFactor) + loopsMNK.k() % splitkFactor;
            kIdxSliceLen = loopsMNK.k() / splitkFactor;
        }
    }
    ...
    {
        return kIdxSliceLen * loopsMNK.m() * loopsMNK.n();
    }
    ...
    {
        uint32_t mnLoops = loopsMNK.m() * loopsMNK.n();
        uint32_t kIdx = kIdxSliceStart + loopIdx / mnLoops;
        uint32_t innerIdx = loopIdx % mnLoops;
