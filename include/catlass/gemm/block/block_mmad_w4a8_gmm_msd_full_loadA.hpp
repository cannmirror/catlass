...
#ifndef CATLASS_GEMM_BLOCK_BLOCK_MMAD_W4A8_GMM_MSD_HPP
#define CATLASS_GEMM_BLOCK_BLOCK_MMAD_W4A8_GMM_MSD_HPP
...
>
struct BlockMmad <
    MmadAtlasA2W4A8GroupedMatmulMSD<
        PRELOAD_STAGES_,
        L1_STAGES_,
        L0A_STAGES_,
        L0B_STAGES_,
...
public:
    // Type Aliases
    using DispatchPolicy = MmadAtlasA2W4A8GroupedMatmulMSD<
        PRELOAD_STAGES_,
        L1_STAGES_,
        L0A_STAGES_,
        L0B_STAGES_,
    ...
    // L1 tile size
    static constexpr uint32_t MAX_FULL_LOAD_K = 1024;
    static constexpr uint32_t L1A_TILE_SIZE = static_cast<uint32_t>(L1TileShape::M * MAX_FULL_LOAD_K * sizeof(int8_t) / 2);
    ...
    CATLASS_DEVICE
    void operator()(
        AscendC::GlobalTensor<uint64_t> const &gmScale, LayoutScale const &layoutScale,
        ...
        GemmCoord const &actualShape, uint32_t quantGroupNum,
        Callback const &callbackBeforeFixpipe, Callback const &callbackAfterFixpipe
    )
    {
        // layoutTileScale: shape{quantGroupNum, L1TileShape::N} stride{quantGroupNum, n}
        auto layoutTileScale = layoutScale.GetTileLayout(MakeCoord(quantGroupNum, L1TileShape::N));
        ...
        // matrix A full load k from GM to L1
        auto layoutTileA = layoutA.GetTileLayout(MakeCoord(actualShape.m(), actualShape.k()));
        LayoutAInL1 fullLoadA1Layout = LayoutAInL1::template MakeLayout<ElementA>(L1TileShape::M, actualShape.k());
        AscendC::SetFlag<AscendC::HardEvent::MTE1_MTE2>(l1AEventList);
        AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1AEventList);
        copyGmToL1A(l1ATensor, gmBlockA, fullLoadA1Layout, layoutTileA);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE1>(l1AEventList);
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_MTE1>(l1AEventList);
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; ++kLoopIdx) {
            ...
            auto layoutKTileScale = layoutScale.GetTileLayout(
                MakeCoord(static_cast<decltype(L1TileShape::N)>(1), L1TileShape::N));
            LayoutL1Scale layoutL1KTileScale{L1TileShape::N};
            MatrixCoord gmKTileScaleOffset{kTileIdx, 0};
            auto gmKTileScale = gmScale[layoutTileScale.GetOffset(gmKTileScaleOffset)];
            AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(l1ScaleEventList[l1ListId]);
            copyGmToL1Scale(l1ScaleBufList[l1ListId], gmKTileScale, layoutL1KTileScale, layoutKTileScale);
            ...
            l1TileMmadParams.kLoopIdx = kLoopIdx;
            ...
            l1TileMmadParams.fullLoadA1Layout = fullLoadA1Layout;
...
private:
    ...
    void InitL1(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart)
    {
        ...
        l1ATensor = resource.l1Buf.template GetBufferByByte<ElementA>(l1AOffset);
        ...
        for (uint32_t i = 0; i < L1_STAGES; ++i) {
            ...
            l1ScaleEventList[i] = i + L1_STAGES * 2;
    ...
    void L1TileMmad(L1TileMmadParams const &params)
    {
        ...
        auto &l1ScaleBuf = l1ScaleBufList[params.l1ListId];
        ...
        for (uint32_t mPartIdx = 0; mPartIdx < mPartLoop; ++mPartIdx) {
            ...
            for (uint32_t kPartIdx = 0; kPartIdx < kPartLoop; ++kPartIdx) {
                ...
                MatrixCoord l1AOffset{mPartIdx * L0TileShape::M,
                                      params.kLoopIdx * L1TileShape::K + kPartIdx * L0TileShape::K};
                auto l1ATile = l1ATensor[params.fullLoadA1Layout.GetOffset(l1AOffset)];

                AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(l0AEventList[l0AListId]);
                copyL1ToL0A(l0ATile, l1ATile, layoutAInL0, params.fullLoadA1Layout);
        ...
        if (params.isKLoopFirst) {
            if constexpr (!ENABLE_UNIT_FLAG) {
                ...
                AscendC::SetFlag<AscendC::HardEvent::FIX_MTE2>(l1ScaleEventList[l1ListId]);
    ...
    L1TileMmadParams l1TileMmadParams;
...
#endif  // CATLASS_GEMM_BLOCK_BLOCK_MMAD_W4A8_GMM_MSD_HPP
