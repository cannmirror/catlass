...
>
struct BlockMmad <
    MmadAtlasA2W4A8GroupedMatmulMSDFullLoadA<
        PRELOAD_STAGES_,
        L1_STAGES_,
        L0A_STAGES_,
        L0B_STAGES_,
...
public:
    // Type Aliases
    using DispatchPolicy = MmadAtlasA2W4A8GroupedMatmulMSDFullLoadA<
        PRELOAD_STAGES_,
        L1_STAGES_,
        L0A_STAGES_,
        L0B_STAGES_,
    ...
    // L1 tile size
    static constexpr uint32_t MAX_K = 1024; // Max k in full load A1 cases.
    static constexpr uint32_t L1A_TILE_SIZE = L0TileShape::M * MAX_K * sizeof(int8_t) / 2;
    ...
    // Check L1TileShape
    static_assert((L1A_TILE_SIZE + L1B_TILE_SIZE) * L1_STAGES + SCALE_KTILE_SIZE <= ArchTag::L1_SIZE,
    ...
    ~BlockMmad()
    {
        ...
        AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(scaleEvent);
    ...
    )
    {
        ...
        // full load matrix A tile from GM to L1
        auto fullLoadALayout = LayoutAInL1::template MakeLayout<ElementA>(L1TileShape::M, actualShape.k());
        ...
        copyGmToL1A(l1ATensorList[0], gmBlockA, L1A_LAYOUT, layoutTileA);
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; ++kLoopIdx) {
            ...
            AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(scaleEvent);
            copyGmToL1Scale(l1ScaleBuf, gmKTileScale, layoutL1KTileScale, layoutKTileScale);
            AscendC::SetFlag<AscendC::HardEvent::MTE2_FIX>(scaleEvent);

            // Copy scale from L1 to FixpipeBuffer
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_FIX>(scaleEvent);
            LayoutFpBuf layoutFpBuf{L1TileShape::N};
            copyL1ToFP(fixpipeBuf, l1ScaleBuf, layoutFpBuf, layoutL1KTileScale);
...
private:
    ...
    void InitL1(Arch::Resource<ArchTag> &resource, uint32_t l1BufAddrStart)
    {
        ...
        l1ScaleBuf = resource.l1Buf.template GetBufferByByte<uint64_t>(l1ScaleOffset);
        AscendC::SetFlag<AscendC::HardEvent::FIX_MTE2>(scaleEvent);
    }
    ...
    AscendC::LocalTensor<uint64_t> l1ScaleBuf;
    AscendC::LocalTensor<uint64_t> fixpipeBuf;
    int32_t scaleEvent{0};
