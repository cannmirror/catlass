...
public:
        ...
        TensorBias const &tensorBias = {})
    {
        ...
        auto layoutInL0C = tla::MakeLayoutL0C(mL1Round, nL1Round);
        ...
        if constexpr (HAS_BIAS && !std::is_same_v<TensorBias, EmptyClass>) {
            ...
            AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE1>(EVENT_ID4);
        }
        ...
        uint32_t mL0Loop = CeilDiv<L0_TILE_M>(mL1Round);
        uint32_t nL0Loop = CeilDiv<L0_TILE_N>(nL1Round);
        ...
        for (uint32_t kL1Idx = 0; kL1Idx < kL1Loop; kL1Idx++) {
            ...
            for (int mL0Idx = 0; mL0Idx < mL0Loop; mL0Idx++) {
                uint32_t mL0Actual = (mL0Idx < mL0Loop - 1) ? L0_TILE_M : (mL1Round - mL0Idx * L0_TILE_M);
                ...
                for (int kL0Idx = 0; kL0Idx < kL0Loop; kL0Idx++) {
                    ...
                    for (int nL0Idx = 0; nL0Idx < nL0Loop; nL0Idx++) {
                        uint32_t nL0Actual = (nL0Idx < nL0Loop - 1) ? L0_TILE_N : (nL1Round - nL0Idx * L0_TILE_N);
                        ...
                        if constexpr (HAS_BIAS && !std::is_same_v<TensorBias, EmptyClass>) {
                            if (initC) {
                                if (nL0Idx == 0) {
                                    AscendC::WaitFlag<AscendC::HardEvent::MTE2_MTE1>(EVENT_ID4);
                                }
                                AscendC::WaitFlag<AscendC::HardEvent::M_MTE1>(EVENT_ID4);
                                auto l1Bias = l1BiasTensor.template ReinterpretCast<ElementBias>();
                                auto tensorL1Bias = tla::MakeTensor(l1Bias, L1BIAS_LAYOUT, Arch::PositionL1{});
