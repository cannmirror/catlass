...
template <class ArchTag_, typename T_, bool WITH_BETA, bool FastComputeMode = false,
...
public:
    ...
    static const int32_t NUM_PER_REP_FP32 = 64; // ONE_REPEAT_BYTE_SIZE / sizeof(float);
    static const uint32_t HIDDTEN_STATE = 7168;
    static const uint32_t MM1_OUT_SIZE = 2112;   // TODO 这个长度和用例里的不一样
    ...
    static const uint32_t REPEAT_TIME_256 = 256; // 128 default stride
    static const uint32_t REPEAT_TIME_128 = 128; // 128 default stride
                                ...
                                uint32_t row_work_, uint32_t n)
    {
        ...
        while (pid < row_work_) {
            ...
            if constexpr (!NEED_DEQUANT) {
                ...
                AscendC::DataCopyPad(perTokenDescaleTensor, perTokenDescaleGmTensor[pid], 
            ...
            AscendC::PipeBarrier<PIPE_V>();
            cce::printf("num_col_ = %d, input_stride_ = %d, avg_factor_ = %f\n", num_col_, input_stride_, avg_factor_);
            ...
            /* Quant start */
            if constexpr (quantMode == QuantMode::PER_TENSOR_ASYMM_QUANT) {
                cce::printf("quantMode == QuantMode::PER_TENSOR_ASYMM_QUANT\n");
                ...
                cce::printf("quantMode == QuantMode::PER_TOKEN_SYMM_QUANT\n");
                ...
                if constexpr (!NEED_DEQUANT) {
                    AscendC::DataCopyPad(perTokenDescaleGmTensor[pid], perTokenDescaleTensor, AscendC::DataCopyExtParams(1, sizeof(float), 0, 0, 0));
                } else {
                    AscendC::DataCopyPad(perTokenDescaleGmTensor[num_row_ + pid], perTokenDescaleTensor, AscendC::DataCopyExtParams(1, sizeof(float), 0, 0, 0));
                }
