...
## Example组装
### 构造输入
- 计算各输入[输入数据len](../../../examples/10_grouped_matmul_slice_m_per_token_dequant/grouped_matmul_slice_m_per_token_dequant.cpp#L82)
- 计算各输入[输入数据size](../../../examples/10_grouped_matmul_slice_m_per_token_dequant/grouped_matmul_slice_m_per_token_dequant.cpp#L88)
- [host侧生成各输入初始值](../../../examples/10_grouped_matmul_slice_m_per_token_dequant/grouped_matmul_slice_m_per_token_dequant.cpp#L94)，这里groupList（M轴前缀和）使用[GenerateGroupList()](../common/golden/fill_data.hpp#L70)随机生成
...
- 定义后处理计算使用的Tile-[TileOneBlkColumnBroadcastMul](../../../examples/10_grouped_matmul_slice_m_per_token_dequant/grouped_matmul_slice_m_per_token_dequant.cpp#L179)：单个Tile块<m0,n0>和perTokenScale的Broadcast片段<m0,blk>，先将perTokenScale片段<m0,blk>Broadcast到<m0,n0>，再做elementwise乘法。和上面TileBroadcastOneBlk绑定使用
...
### 精度校验和空间释放
- 将算子输出[搬回host侧](../../../examples/10_grouped_matmul_slice_m_per_token_dequant/grouped_matmul_slice_m_per_token_dequant.cpp#L213)
