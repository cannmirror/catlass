# Catlass Epilogue Visitor Graph（EVG）设计文档

## 太长不看（TL;DR）

**EVG（Epilogue Visitor Graph）** 是 Catlass 中用于 GEMM 后处理（Epilogue）的声明式框架。它将后处理操作（如加法、类型转换、广播、规约等）抽象为可组合的模板节点，通过树形或拓扑结构拼接，形成计算图。开发者只需用"表达式"声明计算逻辑（如 `D = C + X`），框架自动处理数据搬运、UB 空间分配、事件同步和流水调度。相比手工组织 GM/UB 拷贝和事件同步，EVG 显著降低开发复杂度，同时以尽量保持相近性能为目标，并支持图和节点复用与灵活扩展。
...
- Kernel 将 `problemShape, ptrA, ptrB, evg_args` 打包为 `MatmulVisitor::Arguments`；
- 计算 `GetWorkspaceSize`（= GEMM 的 C 工作区 + EVG 的 workspace 对齐和），调用initalize_workspace()；
- 调用 `ToUnderlyingArguments`：先组织 A/B/C 的布局与 workspace，再用 `EVG::to_underlying_arguments` 生成 Fusion 层 `Params`；
...
- 解释：
  ...
  - `/2`：双缓存
  - `/sizeof(elementType)`：按元素数量计量而非字节，可能会有混合类型的EVG。
...
- 构造 `BlockEpilogue(resource, {evg_params})` 时，分配事件ID并设置初始 flag（允许 MTE2 搬入与 V 写出）；
