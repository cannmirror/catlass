...
本文档对比了 Matmul+Add 的两种实现方式：**未使用 EVG** 需要手工组织 GM/UB 拷贝、事件等待/置位、UB 布局、块/子块切分，并显式调用 `TileElemWiseAdd`；**使用 EVG** 只需用树形 Visitor 声明 `D = C + X`，框架自动处理数据流与事件。性能测试显示两者性能基本相当（平均 speedup ≈ 0.998），但 EVG 方式开发便捷性显著提升，易于扩展为多算子链路（如 Add+Relu）。
