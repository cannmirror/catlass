...
**EVG（Epilogue Visitor Graph）** 是 Catlass 中用于 GEMM 后处理（Epilogue）的声明式框架。它将后处理操作（如加法、类型转换、广播、规约等）抽象为可组合的模板节点，通过树形或拓扑结构拼接，形成计算图。开发者只需用"表达式"声明计算逻辑（如 `D = C + X`），框架自动处理数据搬运、UB 空间分配、事件同步和流水调度。

本文档对比了 Matmul+Add 的两种实现方式：**未使用 EVG** 需要手工组织 GM/UB 拷贝、事件等待/置位、UB 布局、块/子块切分，并显式调用 `TileElemWiseAdd`；**使用 EVG** 只需用树形 Visitor 声明 `D = C + X`，框架自动处理数据流与事件。性能测试显示两者性能基本相当（平均 speedup ≈ 0.996），但 EVG 方式开发便捷性显著提升，易于扩展为多算子链路（如 Add+Relu）。
...
- `include/catlass/epilogue/fusion/operations.hpp` 中 `Add`
...
要点：
- 以 Visitor/Fusion 方式描述 D = C + X：`AccLoad(C) + AuxLoad(X)` → `Add` → `AuxStore(D)`；
- `Add` 的实现依旧是底层 `AscendC::Add`，但数据流/事件由框架拼装完成；
...
- EVG：以 Visitor Tree 声明式拼装，框架负责数据流与事件；易于扩展与维护，可复用算子节点（如 `Add`、`Mul`、`Activation`）。
