...
class PaddingSingleCoreSplitkMatmulTemplate:
...
{{
    ...
    LaunchPaddingSingleCoreSplitkMatmulKernel<ArchTag, ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC,
...
{{
    ...
    constexpr PaddingTag paddingTagB = {padding_tag_b};
    constexpr PaddingTag paddingTagC = {padding_tag_c};
    return PaddingSingleCoreSplitkMatmulKernelGetWorkspaceSize<ArchTag, ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC,
        paddingTagA, paddingTagB, paddingTagC>(tilingParams);
}}
"""
    KERNEL_NAME = "PaddingSingleCoreSplitkMatmulKernel"

    @staticmethod
    def gen_code(dtype, kernel_info):

        kernel_serial = Config.KERNEL_SERIAL_MAP[PaddingSingleCoreSplitkMatmulTemplate.KERNEL_NAME]
        ...
        for l_tag_a, l_tag_b, p_tag_a, p_tag_b, p_tag_c in combinations:
            # kernel_fun_name can be PaddingSingleCoreSplitkMatmulHalfLayout00
            kernel_func_name = (
                PaddingSingleCoreSplitkMatmulTemplate.KERNEL_NAME
                + dtype.capitalize()
                + "Layout"
                + str(l_tag_a)
                + str(l_tag_b)
            ...
            # launch_kernel_fun_name can be LaunchPaddingSingleCoreSplitkMatmulHalfLayout00
            launch_kernel_func_name = "Launch" + kernel_func_name
            # get_workspace_fun_name can be PaddingSingleCoreSplitkMatmulHalfLayout00GetWorkspaceSize
            ...
            content = PaddingSingleCoreSplitkMatmulTemplate.TEMPLATE.format(
