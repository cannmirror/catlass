...
{{
    ...
    constexpr PaddingTag paddingTagA = {padding_tag_a};
    constexpr PaddingTag paddingTagB = {padding_tag_b};
    return PaddingMultiCoreSplitkMatmulKernelGetWorkspaceSize<ArchTag, ElementA, LayoutA, ElementB, LayoutB, ElementC, 
...
"""
    ...
    def gen_code(kernel_name, base_file_name, kernel_serial, dtype, kernel_info):
        ...
        for l_tag_a, l_tag_b, p_tag_a, p_tag_b in combinations:
            ...
            kernel_func_name = (
                PaddingMultiCoreSplitkMatmulTemplate.KERNEL_NAME
                + dtype.capitalize()
                + "Layout"
                + str(l_tag_a)
                + str(l_tag_b)
