...
class PaddingSingleCoreSplitkKLoopOuterMatmulTemplate:

    TEMPLATE = """
#include "kernel/padding_single_core_splitk_matmul_kernel.h"
...
{{
    ...
    LaunchPaddingSingleCoreSplitkKLoopOuterMatmulKernel<ArchTag, ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC,
...
{{
    ...
    constexpr PaddingTag paddingTagB = {padding_tag_b};
    constexpr PaddingTag paddingTagC = {padding_tag_c};
    return PaddingSingleCoreSplitkKLoopOuterMatmulKernelGetWorkspaceSize<ArchTag, ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC,
        paddingTagA, paddingTagB, paddingTagC>(tilingParams);
}}
"""
    KERNEL_NAME = "PaddingSingleCoreSplitkKLoopOuterMatmulKernel"

    @staticmethod
    def gen_code(dtype, kernel_info):

        kernel_serial = Config.KERNEL_SERIAL_MAP[PaddingSingleCoreSplitkKLoopOuterMatmulTemplate.KERNEL_NAME]
        ...
        for l_tag_a, l_tag_b, p_tag_a, p_tag_b, p_tag_c in combinations:
            # kernel_fun_name can be PaddingSingleCoreSplitkKLoopOuterMatmulHalfLayout00
            kernel_func_name = (
                PaddingSingleCoreSplitkKLoopOuterMatmulTemplate.KERNEL_NAME
                + dtype.capitalize()
                + "Layout"
                + str(l_tag_a)
                + str(l_tag_b)
            ...
            # launch_kernel_fun_name can be LaunchPaddingSingleCoreSplitkKLoopOuterMatmulHalfLayout00
            launch_kernel_func_name = "Launch" + kernel_func_name
            # get_workspace_fun_name can be PaddingSingleCoreSplitkKLoopOuterMatmulHalfLayout00GetWorkspaceSize
            get_workspace_func_name = kernel_func_name + "GetWorkspaceSize"
            # file name can be padding_single_core_splitk_k_loop_outer_matmul_kernel_half_layout_00.cpp
            ...
            content = PaddingSingleCoreSplitkKLoopOuterMatmulTemplate.TEMPLATE.format(
