...
#ifndef PADDING_HYBRID_SPLITK_MATMUL_KERNEL
#define PADDING_HYBRID_SPLITK_MATMUL_KERNEL
...
#include "catlass/gemm/kernel/dynamic_padding_hybrid_splitk_matmul.hpp"
...
[[bisheng::core_ratio(1, 2)]] CATLASS_GLOBAL void PaddingHybridSplitkMatmulKernel(uint64_t fftsAddr, __gm__ uint8_t *__restrict__ gmA,
    ...
    __gm__ uint8_t *__restrict__ gmWB, __gm__ uint8_t *__restrict__ gmWC, __gm__ uint8_t *__restrict__ tilingData)
{
     ...
     * | 46-47  | 2    | splitkFactor| uint16_t  | splitk factor                 |
    ...
    *(uint64_t *)(tilingParams + 44) = *(reinterpret_cast<__gm__ uint64_t *>(tilingData + 48));
     ...
     * | 42-43  | 2    | splitkFactor | uint16_t  | tilingParams[42:43]| splitk factor      |
    ...
    constexpr uint32_t computeLength = 32 * 1024 / sizeof(ElementAccumulator);
    using RemovePaddingNDReduceAdd = Catlass::Gemm::Kernel::RemovePaddingNDReduceAdd<
                    paddingTagC, ArchTag, ElementAccumulator, ElementC, LayoutC, computeLength>;
    ...
    if (problemShape.m() > problemShape.n()) {
        ...
        using BlockScheduler = typename Catlass::Gemm::Block::HybridSplitkGemmIdentityBlockSwizzle<8, 0>;
        // kernel level
        using MatmulKernel = Catlass::Gemm::Kernel::DynamicPaddingHybridSplitkMatmul<
            PaddingA, PaddingB, BlockMmad, BlockEpilogue, BlockScheduler, RemovePaddingNDReduceAdd>;
        typename MatmulKernel::Params params{
            problemShape, l1TileShape, l0TileShape, gmA, layoutA, gmB, layoutB, gmC, layoutC, gmWA, gmWB, gmWC, splitkFactor};
        // call a kernel
        MatmulKernel matmul;
        ...
        using BlockScheduler = typename Catlass::Gemm::Block::HybridSplitkGemmIdentityBlockSwizzle<8, 1>;
        // kernel level
        using MatmulKernel = Catlass::Gemm::Kernel::DynamicPaddingHybridSplitkMatmul<
            PaddingA, PaddingB, BlockMmad, BlockEpilogue, BlockScheduler, RemovePaddingNDReduceAdd>;
        typename MatmulKernel::Params params{
...
template <class ElementA, class LayoutA, class ElementB, class LayoutB, class ElementC, class LayoutC,
    PaddingTag paddingTagA, PaddingTag paddingTagB,  PaddingTag paddingTagC>
void LaunchPaddingHybridSplitkMatmulKernel(aclrtStream &stream, uint64_t fftsAddr, uint8_t *dA, uint8_t *dB, uint8_t *dC,
    uint8_t *dW, uint8_t *dTilingParams, TilingParams &tilingParams)
{
    ...
    PaddingHybridSplitkMatmulKernel<ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC, paddingTagA, paddingTagB, paddingTagC>
        <<<tilingParams.blockDim, nullptr, stream>>>(fftsAddr, dA, dB, dC, dWA, dWB, dWC, dTilingParams);
}

template <class ElementA, class LayoutA, class ElementB, class LayoutB, class ElementC, class LayoutC,
    PaddingTag paddingTagA, PaddingTag paddingTagB,  PaddingTag paddingTagC>
size_t PaddingHybridSplitkMatmulKernelGetWorkspaceSize(TilingParams &tilingParams)
{
    using ArchTag = Catlass::Arch::AtlasA2;
    using PaddingBuilderA = Catlass::Gemm::Kernel::PaddingBuilder<paddingTagA, ArchTag, ElementA, LayoutA>;
    using PaddingBuilderB = Catlass::Gemm::Kernel::PaddingBuilder<paddingTagB, ArchTag, ElementB, LayoutB>;
    ...
    uint32_t n1 = static_cast<uint32_t>(tilingParams.n1);
    uint32_t k1 = static_cast<uint32_t>(tilingParams.k1);
    uint32_t splitkFactor = static_cast<uint32_t>(tilingParams.splitkFactor);
    ...
    if constexpr (paddingTagC == PaddingTag::NO_PADDING) {
        sizeWC = RemovePaddingNDReduceAdd::GetWorkspaceSize(m, n, splitkFactor);
    } else if constexpr (paddingTagC == PaddingTag::PADDING_ND) {
        // Optimal bandwidth for 512 Byte aligned writes
        sizeWC = RemovePaddingNDReduceAdd::GetWorkspaceSize(m, n, splitkFactor, 512 / sizeof(ElementAccumulator));
    }
    return sizeWA + sizeWB + sizeWC;
}

#endif  // PADDING_HYBRID_SPLITK_MATMUL_KERNEL