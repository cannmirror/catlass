...
class test_kvcache:
    ...
    def calc_vec_mm_data(self, N, headNum, data_type, quant_mode):
        ...
        mm1OutSplit1.numpy().tofile("data/s2.bin")
        print("s2_shape={}, dtype={}".format(mm1OutSplit1.shape, mm1OutSplit1.dtype))

        bias_11, bias_12 = torch.split(self.bias1, [576, 1536], dim=1)  # [1, 576], [1, 1536]

        bias_11.numpy().tofile("data/bias_11.bin")
        ...
        if quant_mode == 0:
            ...
            perTokenDescale1.numpy().tofile("data/s5.bin")
            print("s5_shape={}, dtype={}".format(perTokenDescale1.shape, perTokenDescale1.dtype))
        ...
        mm11OutSplit1.numpy().tofile("data/mm11OutSplit1.bin")
        print("mm11OutSplit1_shape={}, dtype={}".format(mm11OutSplit1.shape, mm11OutSplit1.dtype))
        mm12OutSplit1.numpy().tofile("data/mm12OutSplit1.bin")
        ...
        result1.numpy().tofile("data/keycache1.bin")
        result2 = self.keyOut1[..., 512:576]
        result2.numpy().tofile("data/keycache2.bin")
    ...
    def calc_vec_mm_atb_data(self, N, headNum, data_type, cacheMode, quant_mode=0):
        ...
        self.gamma3.numpy().tofile("data/gamma3.bin")
        print("gamma3_shape={}, dtype={}".format(self.gamma3.shape, self.gamma3.dtype))

        self.sin1 = torch.rand(size=(N, 64)).to(data_type)
        self.cos1 = torch.rand(size=(N, 64)).to(data_type)

        self.cos1.numpy().tofile("data/cos1.bin")
        self.sin1.numpy().tofile("data/sin1.bin")
        ...
        self.keyCache.numpy().tofile("data/keyCache.bin")

        self.slotMapping = torch.from_numpy(np.random.choice(192 * 128, N, replace=False).astype(np.int32)).to(
            torch.int32
        )
        self.slotMapping[0] = -1
        self.slotMapping.numpy().tofile("data/slotMapping.bin")
        print("slotMapping={}, dtype={}".format(self.slotMapping.shape, self.slotMapping.dtype))
