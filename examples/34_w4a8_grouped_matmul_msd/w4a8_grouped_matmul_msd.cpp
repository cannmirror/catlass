...
void Run(Options const &options)
{
    ...
    LayoutB layoutB{k, n};
    LayoutScale layoutScale{quantGroupNum * problemCount};
    ...
    std::string inputGroupList_path = "../../examples/34_w4a8_grouped_matmul_msd/data/inputGroupList.dat";
    ReadFile(inputGroupList_path, hostGroupList, sizeGroupList);

    void *hostA = nullptr;
    ACL_CHECK(aclrtMallocHost(&hostA, sizeA));
    std::string inputA_path = "../../examples/34_w4a8_grouped_matmul_msd/data/inputA.dat";
    ReadFile(inputA_path, hostA, sizeA);

    void *hostB = nullptr;
    ACL_CHECK(aclrtMallocHost(&hostB, sizeB));
    std::string inputB_path = "../../examples/34_w4a8_grouped_matmul_msd/data/inputB.dat";
    ReadFile(inputB_path, hostB, sizeB);

    void *hostScale = nullptr;
    ACL_CHECK(aclrtMallocHost(&hostScale, sizeScale));
    std::string inputScale_path = "../../examples/34_w4a8_grouped_matmul_msd/data/inputScale.dat";
    ReadFile(inputScale_path, hostScale, sizeScale);

    void *hostBias = nullptr;
    ACL_CHECK(aclrtMallocHost(&hostBias, sizeBias));
    std::string inputBias_path = "../../examples/34_w4a8_grouped_matmul_msd/data/inputBias.dat";
    ...
    std::string inputPerTokenScale_path = "../../examples/34_w4a8_grouped_matmul_msd/data/inputPerTokenScale.dat";
    ReadFile(inputPerTokenScale_path, hostPerTokenScale, sizePerTokenScale);

    std::vector<float> hExpected(goldenSize);
    std::string expected_path = "../../examples/34_w4a8_grouped_matmul_msd/data/expected.dat";
...
//-------------------------------------------------------
    ...
    using TileRowBroadcastMul = Epilogue::Tile::TileRowBroadcastAdd<ArchTag, RowBroadcastMulType, EpilogueTileShape>;
    ...
    // kernel level
    using MatmulKernel = Gemm::Kernel::W4A8GroupedMatmulMSD<BlockMmad, BlockEpilogue, BlockScheduler>;
    // call a kernel
