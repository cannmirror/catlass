...
与`37_a5_tail_multi_core_splitk_matmul`不同的是，该模板切分不在通过一个切分因子切分`K`，而是将尾轮的计算量直接均分到所有计算核心上。

```sh
# 编译指定用例
bash scripts/build.sh 38_a5_streamk_matmul -DCATLASS_BISHENG_ARCH=a5
cd output/bin
# 可执行文件名 |矩阵m轴|n轴|k轴|Device ID
# Device ID可选，默认为0
./38_a5_streamk_matmul 256 512 1024 0
```
执行结果如下，说明精度比对成功。
```
Compare success.
