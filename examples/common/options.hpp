...
/**
 * @struct GroupedGemmWithPerGroupOptions
 * @brief Options structuture for quant grouped/batched gemm with per group dequant examples.
 * @brief Arguments: `example_name problem_count k_group_size m n k [device_id]`
 */
struct GroupedGemmWithPerGroupOptions {
    const std::string HELPER = "problem_count k_group_size m n k [device_id]";

    Catlass::GemmCoord problemShape{128, 128, 128};
    uint32_t problemCount{1};
    uint32_t kGroupSize{512};
    int32_t deviceId{0};

    GroupedGemmWithPerGroupOptions() = default;
    ...
    int Parse(int argc, const char **argv) {
        ...
        problemCount = std::atoi(argv[static_cast<uint32_t>(ArgsIndex::GROUP_COUNT)]);
        kGroupSize = std::atoi(argv[static_cast<uint32_t>(ArgsIndex::K_GROUP_SIZE)]);
        problemShape.m() = std::atoi(argv[static_cast<uint32_t>(ArgsIndex::M_INDEX)]);
        problemShape.n() = std::atoi(argv[static_cast<uint32_t>(ArgsIndex::N_INDEX)]);
        problemShape.k() = std::atoi(argv[static_cast<uint32_t>(ArgsIndex::K_INDEX)]);
        if (argc == static_cast<uint32_t>(ArgsIndex::ARGS_MAX)) {
