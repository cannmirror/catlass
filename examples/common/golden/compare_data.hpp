...
template<class ElementResult, class ElementCompare>
std::vector<uint64_t> CompareDataW4A8GmmMSD(const std::vector<ElementResult>& result, const std::vector<ElementCompare>& expect,
    const float &atol, const float &rtol, uint64_t resultNum)
{
    std::vector<uint64_t> errorIndices;
    for (uint64_t i = 0; i < resultNum; ++i) {
        ElementCompare actualValue = static_cast<ElementCompare>(result[i]);
        ElementCompare expectValue = expect[i];
