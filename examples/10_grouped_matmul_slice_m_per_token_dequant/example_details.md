...
- **Example组装**，[grouped_matmul_slice_m_per_token_dequant.cpp](./grouped_matmul_slice_m_per_token_dequant.cpp)；
...
- **Block组件**，包含：
    - 通用的mmad组件[block_mmad_preload_async_with_callback.hpp](../../include/catlass/gemm/block/blockMmad_preload_async_with_callback.hpp)
...
- **Tile组件**，除一些基本Tile_copy和tile_mmad组件外，重点关注组件：
    - blockMmad中CopyGmToL1A使用的[CopyGmToL1GMMPTD](../../include/catlass/gemm/tile/copy_gm_to_l1.hpp)
...
## Example组装
### 构造输入
- 计算各输入[输入数据len](./grouped_matmul_slice_m_per_token_dequant.cpp#L82)
- 计算各输入[输入数据size](./grouped_matmul_slice_m_per_token_dequant.cpp#L88)
- [host侧生成各输入初始值](./grouped_matmul_slice_m_per_token_dequant.cpp#L94)，这里groupList（M轴前缀和）使用[GenerateGroupList()](../common/golden/fill_data.hpp#L70)随机生成
- [构造device侧输入](./grouped_matmul_slice_m_per_token_dequant.cpp#L105)

### 组装blockMmad
- [定义各输入的layout](./grouped_matmul_slice_m_per_token_dequant.cpp#L131)，[layout详见](../../docs/tla/01_layout.md)
- [设置DispatchPolicy](./grouped_matmul_slice_m_per_token_dequant.cpp#L152)为[MmadAtlasA2PreloadAsyncWithCallback](../../docs/dispatch_policies.md#L72)，即选取blockMmad组件
- 设置[L1TileShape和L0TileShape](../../docs/dispatch_policies.md#L154)，用于切基本块分核和L1/L0上切块。需要注意TileShape和DispatchPolicy的设置有约束关系，例如当前L1::M=128、L1::K=128时，l0AStages不能超过4，以免超出L0A大小、在样例编译时被blockMmad侧静态校验拦截。TileShape计算可参考[TileShape约束](../../docs/catlass_optimize_guidance.md)
- [设置blockMmad输入输出Type](../../docs/dispatch_policies.md#L157)
- 为了专门使用CopyGmToL1GMMPTD来做CopyGmToL1A的动作，重新[定义](./grouped_matmul_slice_m_per_token_dequant.cpp#L49)并[组装](./grouped_matmul_slice_m_per_token_dequant.cpp#L161)了新的TileCopyMmad，若不使能CopyGmToL1GMMPTD可以跳过此步骤，BlockMmad会使用默认TileCopyMmad。
- 使用上述模板入参[组装BlockMmad](./grouped_matmul_slice_m_per_token_dequant.cpp#L162)

### 组装blockEpilogue
- [设置EpilogueDispatchPolicy](./grouped_matmul_slice_m_per_token_dequant.cpp#L166)为[EpilogueAtlasA2PerTokenDequant](../../docs/dispatch_policies.md#L72)，即选取block_epilogue组件
- 定义后处理[相关入参的Type](./grouped_matmul_slice_m_per_token_dequant.cpp#L167)：ScaleType、PerTokenScaleType、DType
- 定义后处理[各计算步骤的Type](./grouped_matmul_slice_m_per_token_dequant.cpp#L171)：RowBroadcastMulType、BroadcastOneBlkType、OneBlkColumnBroadcastMulType
- 定义后处理[单次计算的Tile大小](./grouped_matmul_slice_m_per_token_dequant.cpp#L175)：<m0,n0>
- 定义后处理计算使用的Tile-[TileRowBroadcastMul](./grouped_matmul_slice_m_per_token_dequant.cpp#L176)：单个Tile块<m0,n0>和scale片段<1,n0>，先将scale片段<1,n0>Broadcast到<m0,n0>，再做elementwise乘法
- 定义后处理计算使用的Tile-[TileBroadcastOneBlk](./grouped_matmul_slice_m_per_token_dequant.cpp#L177)：perTokenScale片段<m0,1>做Broadcast到<m0, blk>
- 定义后处理计算使用的Tile-[TileOneBlkColumnBroadcastMul](./grouped_matmul_slice_m_per_token_dequant.cpp#L179)：单个Tile块<m0,n0>和perTokenScale的Broadcast片段<m0,blk>，先将perTokenScale片段<m0,blk>Broadcast到<m0,n0>，再做elementwise乘法。和上面TileBroadcastOneBlk绑定使用
- 定义后处理搬运使用的Tile-[TileCopy](./grouped_matmul_slice_m_per_token_dequant.cpp#L181)，[偏特化位置](../../include/catlass/epilogue/tile/tile_copy.hpp#L71)
- 定义后处理计算基本块时、按照<m0,n0>切块后的swizzle策略-[EpilogueHorizontalTileSwizzle](./grouped_matmul_slice_m_per_token_dequant.cpp#L182)，[源码位置](../../include/catlass/epilogue/tile/tile_swizzle.hpp#L55)，目前提供EpilogueIdentityTileSwizzle和EpilogueHorizontalTileSwizzle
- 使用上述模板入参[组装BlockEpilogue](./grouped_matmul_slice_m_per_token_dequant.cpp#L184)

### 组装和执行kernel
- 定义kernel中[基本块的swizzle策略](./grouped_matmul_slice_m_per_token_dequant.cpp#L188)，详见[swizzle_explanation](../../docs/swizzle_explanation.md)
- [组装kernel](./grouped_matmul_slice_m_per_token_dequant.cpp#L192)-GroupedMatmulSliceMPerTokenDequantMultiStageWorkspace，需要引用对应[Kernel文件](./grouped_matmul_slice_m_per_token_dequant.cpp#L29)
- 将kernel[组装](./grouped_matmul_slice_m_per_token_dequant.cpp#L195)入适配器并[实例化](./grouped_matmul_slice_m_per_token_dequant.cpp#L201)
- [构造入参arguments](./grouped_matmul_slice_m_per_token_dequant.cpp#L197)
- [校验入参arguments](./grouped_matmul_slice_m_per_token_dequant.cpp#L202)，当前kernel内无实现、直接返回true
- [计算算子需要的workspace大小](./grouped_matmul_slice_m_per_token_dequant.cpp#L203)，kernel内实现
- [申请workspace](./grouped_matmul_slice_m_per_token_dequant.cpp#L206)
- [适配器初始化算子](./grouped_matmul_slice_m_per_token_dequant.cpp#L208)，特别注意，对于涉及核间同步的算子，需要[初始化fftsAddr](./grouped_matmul_slice_m_per_token_dequant.cpp#L138)并在MatmulAdapter执行时调用传入
```
...
```
- [适配器执行算子](./grouped_matmul_slice_m_per_token_dequant.cpp#L209)
- [流同步](./grouped_matmul_slice_m_per_token_dequant.cpp#L210)

### 精度校验和空间释放
- 将算子输出[搬回host侧](./grouped_matmul_slice_m_per_token_dequant.cpp#L213)
- [计算golden标杆](./grouped_matmul_slice_m_per_token_dequant.cpp#L216)
- [精度比对](./grouped_matmul_slice_m_per_token_dequant.cpp#L221)
- [释放输入输出和workspace](./grouped_matmul_slice_m_per_token_dequant.cpp#L228)
...
flowchart 
    ...
    style B00 fill:#ffcdd2
    style B02 fill:#ffcdd2
    style B03 fill:#ffcdd2
    style C00 fill:#ffcdd2
    style C01 fill:#ffcdd2
    style C03 fill:#ffcdd2
