...
void Run(Options const &options)
{
    ...
    constexpr uint32_t preloadStages = 0;  // 预留 目前暂未实现preload功能
    constexpr uint32_t l1Stages = 1;  // 预留 目前暂未实现l1上多buffer功能
    constexpr uint32_t l0AStages = 2;
    constexpr uint32_t l0BStages = 2;
    constexpr uint32_t l0CStages = 1;
    constexpr uint32_t workspaceStages = 4;
    constexpr bool enableUnitFlag = false;  // 可开
    ...
    using L1TileShape = GemmShape<128, 256, 320>;  // 这里k轴大小要和kGroupSize相等
    ...
    // kernel level
    using MatmulKernel = Gemm::Kernel::W4A8GroupedMatmulMSD<BlockMmad, BlockEpilogue,
        BlockScheduler, workspaceStages, ElementGroupList, ElementScale, LayoutScale>;
    // call a kernel
    ...
    size_t resultNum = static_cast<size_t>(m * n);
    std::vector<uint64_t> errorIndices = golden::CompareDataW4A8GmmMSD(hostD, hExpected, 0.005, 0.005, resultNum);
    size_t errorNum = errorIndices.size();
    float correctRation = 1 - static_cast<float>(errorNum) / resultNum;
    if (correctRation > 0.9) {
