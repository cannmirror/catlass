...
def gen_testcase(path: str, param: OpParam) -> None:
    ...
    assert transA and transB, "Transposition is not supported for matrices A and B"
    assert k % 2 == 0 and n % 2 == 0, "k and n should be even"
    assert k % kGroupSize == 0, "Only supports cases where k divides kGroupSize"
    ...
    perGroupScale = np.random.normal(0, 2, [g, quantGroupNum, n]).astype(np.float32)
