...
def gen_seqlen(max_q_seqlen: int, max_kv_seqlen: int, is_varied_len: int, batch: int):
    ...
    kv_seqlen_list = []
    if is_varied_len == 0:
        q_seqlen_list = [max_q_seqlen] * batch # 每个seq的长度
...
class TestFlashAttentionInfer():
    ...
    def calc_data(self, gen_data_params: GenDataParams):
        ...
        kv_min_range = -1.0
        kv_max_range = 1.0
        num_tokens = np.array(gen_data_params.q_seqlen_list).sum() # 多个查询序列(目标)的token总数
        num_kv_tokens = np.array(gen_data_params.k_seqlen_list).sum() # 多个键值序列(源)的token总数
        batch_size = len(gen_data_params.q_seqlen_list) # batch是batch_size
        # 生成所有token的完整query embedding
        ...
        if gen_data_params.kv_dtype == 1:
            ...
            # blockNum * blocksize * KvHeadNums * Embedding_Size
            ...
            # blockNum * blocksize * KvHeadNums * Embedding_Size
            max_num_blocks_per_seq = (max_k_seqlen + gen_data_params.block_size - 1) // gen_data_params.block_size # 每个句子按128(token)划分，能分多少块
            for i in range(batch_size):
                block_table = [
                    max_num_blocks_per_seq * i + j
                    for j in range(max_num_blocks_per_seq)
                ] # [max_num_blocks_per_seq*i+0, +1, ...]
                block_tables.append(block_table) # [[seq1的block块idx],[seq2的block块idx],...]
        elif gen_data_params.kv_dtype == 0:
            if layout == 'TND': # What's the meaning of TND\BSND?
                key_cache = np.random.uniform(kv_min_range, kv_max_range,
        ...
        if gen_data_params.mask_type == 1:
            mask = np.zeros(shape=(num_tokens, max_k_seqlen)).astype(gen_data_params.dtype) # Total_Q_TokenNums * maxK_Seqlen ?
            pre_qseqlen = 0
            for i in range(batch_size):
                qseqlen = gen_data_params.q_seqlen_list[i]
                kseqlen = gen_data_params.k_seqlen_list[i]
                tri = np.ones((qseqlen, qseqlen)) # qseqlen*qseqlen的全1矩阵
                tri = np.triu(tri, 1).astype(gen_data_params.dtype) # np.triu 取上三角矩阵
                mask[pre_qseqlen : (pre_qseqlen + qseqlen), kseqlen - qseqlen : kseqlen] = tri # 把mask对应的这些token部分替换为tri
                pre_qseqlen += qseqlen
...
if __name__ == "__main__":
    ...
    num_blocks = batch * ((max_kv_seqlen + block_size - 1) // block_size) # kv总的Block数
