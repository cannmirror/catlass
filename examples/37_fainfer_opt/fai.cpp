...
struct Options {
    ...
    uint32_t batch{0}; // batch_size
    uint32_t qSeqlen{0}; // Q token nums
    uint32_t kvSeqlen{0}; // KV token nums
    uint32_t numHeads{0}; // Q numHeads
    uint32_t kvHeads{0}; // Kv Heads, MHA(KvHeads = numHeads), GQA(1<kvHeads<numHeads)
    uint32_t embeddingSize{0}; // d-dimention of each head embedding
    uint32_t isVariedLen{0}; // Q,K,V是否会有不同长度的序列
    uint32_t maskType{0}; // what is this?
    uint32_t deviceId{0};
    uint32_t blockSize{128};
    string dataType = "half";
    string dataPath = "../../examples/32_flash_attention_infer_opt/data";
...
void Run(const Options &options)
{
    ...
    ReadFile(dataPath + "/q_ntokens.bin", qNtokens, 1 * sizeof(int32_t)); // 所有Query 的Token总数
    int32_t numTokens = static_cast<int32_t *>(qNtokens)[0];

    uint64_t seqArraySize = batch * sizeof(int64_t);
    uint64_t qoSize = (uint64_t)numTokens * (uint64_t)numHeads * (uint64_t)embeddingSize * sizeof(fp16_t); // 总的Query对应的 q，o的size
    uint64_t kvSize =
        (uint64_t)numBlocks * (uint64_t)blockSize * (uint64_t)kvHeads * (uint64_t)embeddingSize * sizeof(fp16_t);
    uint64_t maskSize = 1024 * 1024 * sizeof(fp16_t); // 2MB?
    uint64_t blockTableSize =
        static_cast<uint64_t>(batch * ((maxKvSeqlen + blockSize - 1) / blockSize) * sizeof(int32_t)); // block table 里有多少个block，所需的分配空间
    // ?????
    ...
    uint8_t *qSeqHost;
    uint8_t *qSeqDevice;
    AllocMem(&qSeqHost, &qSeqDevice, seqArraySize);  // Host侧和Device侧都分配？
    ReadFile(dataPath + "/q_seqlen.bin", qSeqHost, seqArraySize); // batch个qSeq的长度搬过来
    ACL_CHECK(aclrtMemcpy(qSeqDevice, seqArraySize, qSeqHost, seqArraySize, ACL_MEMCPY_HOST_TO_DEVICE)); // 搬到Device上

    // Allocate matrices in host and device memory.
    uint8_t *kvSeqHost;
    uint8_t *kvSeqDevice;
    AllocMem(&kvSeqHost, &kvSeqDevice, seqArraySize);
    ReadFile(dataPath + "/kv_seqlen.bin", kvSeqHost, seqArraySize); // 同理搬过来batch个kv的seqlen
    ...
    ReadFile(dataPath + "/q.bin", qHost, qoSize); // 搬入query，所有token的完整query embedding
    ...
    ReadFile(dataPath + "/k.bin", kHost, kvSize); // 搬入 KCache
    ...
    ReadFile(dataPath + "/v.bin", vHost, kvSize); // 搬入 VCache
    ...
    if (maskType == 1) {
        ...
        ReadFile(dataPath + "/mask.bin", maskHost, maskSize); // 搬入Mask
    ...
    ReadFile(dataPath + "/block_table.bin", blockTableHost, blockTableSize); // 搬入 block Table
    ACL_CHECK(aclrtMemcpy(blockTableDevice, blockTableSize, blockTableHost, blockTableSize, ACL_MEMCPY_HOST_TO_DEVICE));

    uint8_t *oDevice{nullptr}; // 分配Device侧结果空间？这个大小怎么确定的？
    ...
    faInfo.numTokens = numTokens; // 所有Query 的Token总数
    faInfo.numHeads = numHeads; // Q numHeads
    faInfo.embeddingSize = embeddingSize; // token 嵌入维度
    faInfo.numBlocks = numBlocks; // batch 个 kv源序列按128token划分块，能划分的总块数
    faInfo.blockSize = blockSize; // 128
    faInfo.kvHeads = kvHeads; // Kv Heads, MHA(KvHeads = numHeads), GQA(1<kvHeads<numHeads)
    faInfo.batch = batch; // 一个batch里的batch_size，序列的数量
    faInfo.maskType = static_cast<FAInferTiling::MaskType>(maskType);
    faInfo.qSeqlenList = reinterpret_cast<int64_t *>(qSeqHost); // 每个Q序列的token长度数组
    faInfo.kvSeqlenList = reinterpret_cast<int64_t *>(kvSeqHost); // 每个kv序列的token长度数组

    FATilingData faTilingData;

    FAInferTiling::GetFATilingParam(faInfo, blockDim, faTilingData); // 填充faTilingData

    // Allocate matrices in device memory for workspace.
    uint8_t *sDevice;
    ACL_CHECK(aclrtMalloc((void **)(&sDevice), faTilingData.mm1OutSize, ACL_MEM_MALLOC_HUGE_FIRST));
    uint8_t *pDevice;
    ACL_CHECK(aclrtMalloc((void **)(&pDevice), faTilingData.smOnlineOutSize, ACL_MEM_MALLOC_HUGE_FIRST));
    uint8_t *oTempDevice;
    ACL_CHECK(aclrtMalloc((void **)(&oTempDevice), faTilingData.mm2OutSize, ACL_MEM_MALLOC_HUGE_FIRST));
    uint8_t *oUpdateDevice;
    ACL_CHECK(aclrtMalloc((void **)(&oUpdateDevice), faTilingData.UpdateSize, ACL_MEM_MALLOC_HUGE_FIRST));
    ...
    for (int i = 0; i < 1; i++) {
        if (dataType == "half") {
            FAInfer<half><<<blockDim, nullptr, stream>>>(fftsAddr,
                qDevice, // 搬入query，所有token的完整query embedding
                kDevice, // 搬入 KCache
                vDevice, // 搬入 VCache
                maskDevice, // 搬入Mask
                blockTableDevice, // [[seq1的block块idx],[seq2的block块idx],...]
                oDevice, // 分配Device侧结果空间？
                qSeqDevice, // batch个qSeq的长度搬过来
                kvSeqDevice, // batch个kvSeq的长度搬过来
                sDevice,
                pDevice,
                oTempDevice,
                oUpdateDevice,
            ...
            FAInfer<bfloat16_t><<<blockDim, nullptr, stream>>>(fftsAddr,
                qDevice,
                kDevice,
                vDevice,
                maskDevice,
